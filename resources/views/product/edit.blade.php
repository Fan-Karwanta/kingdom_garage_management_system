          ...
          <form id="productEdit-Form" method="post" action="update/{{ $product->id }}" enctype="multipart/form-data" class="form-horizontal upperform">
            ...
            <div class="row row-mb-0">
              ...
              <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 {{ $errors->has('price') ? ' has-error' : '' }} my-form-group">
                <label class="control-label col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4" for="first-name">{{ trans('message.Product Cost') }} (<?php echo getCurrencySymbols(); ?>) <label class="color-danger">*</label></label>
                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                  <input type="text" id="price" name="price" class="form-control" value="{{ $product->price }}" placeholder="{{ trans('message.Enter Product Cost') }}" maxlength="10" required>
                  @if ($errors->has('price'))
            ...
            <div class="row">
               <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                ...
                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                  <input type="text" maxlength="3" name="Discount" value="{{ $product->discount }}" class="form-control discount" id="disc">
                </div>
              </div>
              <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                <label class="control-label col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4" for="cus_name">{{ trans('message.Markup Margin') }} (%)</label>
                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                <input type="text" id="margin" name="margin" class="form-control" placeholder="{{ trans('message.Enter Product Margin') }} (ex: 10.5%)" value="{{ $product->margin }}" maxlength="10">
            ...
            <div class="row">
              ...
              @php
                  $selectedTaxes = json_decode($product->tax_id, true) ?? [];
              @endphp
              ...
              <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                    ...
                    <tr>
                        <td>
                            <input type="checkbox"
                                   ...
                                   {{ in_array($taxes->id, $selectedTaxes) ? 'checked' : '' }}>
                            {{ $taxes->taxname }} {{ $taxes->tax }}%
                        </td>
                    </tr>
                @endforeach
                    </tbody>
               ...
               <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6">
                <label class="control-label col-md-4 col-lg-4 col-xl-4 col-xxl-4 col-sm-4 col-xs-4" for="cus_name">{{ trans('message.Selling Price') }}</label>
                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8">
                <input type="text" id="sellingprice" name="sellingprice" class="form-control" value="{{ $product->selling_price }}" maxlength="10" required>
                  @if ($errors->has('price'))
                  <span class="help-block">
                    <strong>{{ $errors->first('price') }}</strong>
                  </span>
...
$(document).ready(function() {
  ...
  // 🔹 Function to calculate selling price based on price, margin, tax, and discount
  function calculateSellingPrice() {
    var basePrice = parseFloat($('#price').val()) || 0;
    if (basePrice <= 0) return;

    // --- Margin ---
    var marginPercent = parseFloat($('#margin').val()) || 0;
    if (marginPercent < 0) marginPercent = 0;

    // Calculate margin amount and add to base price
    var marginAmount = (basePrice * marginPercent) / 100;
    var priceWithMargin = basePrice + marginAmount;

    // --- Tax ---
    var totalTaxPercent = 0;
    $('.check_tax:checked').each(function() {
      var taxRate = parseFloat($(this).attr('taxrate')) || 0;
      totalTaxPercent += taxRate;
    });
    var taxAmount = (priceWithMargin * totalTaxPercent) / 100;
    var priceWithTax = priceWithMargin + taxAmount;

    // --- Discount ---
    var discountPercent = parseFloat($('#disc').val()) || 0;
    if (discountPercent > 100) discountPercent = 100;
    if (discountPercent < 0) discountPercent = 0;

    var discountAmount = (priceWithTax * discountPercent) / 100;
    var sellingPrice = priceWithTax - discountAmount;
  ...
  $('#price, #margin, #disc').on('keyup change', function() {
    var basePrice = parseFloat($('#price').val()) || 0;
    if (basePrice > 0) calculateSellingPrice();
    else $('#sellingprice').val('');
  });

  $('.check_tax').on('change', function() {
    var basePrice = parseFloat($('#price').val()) || 0;
    if (basePrice <= 0) {
      $(this).prop('checked', false);
      swal({
        title: "{{ trans('message.Price Required') }}",
        text: "{{ trans('message.Please enter product price first before applying tax') }}",
        icon: 'warning',
        cancelButtonColor: '#C1C1C1',
  ...
  // 🔹 Validation for numeric input with decimals (price & margin)
  $('#price, #margin, #sellingprice').on('keypress', function(e) {
  ...
  // 🔹 Limit margin to 2 decimal digits
  ...
  $('body').on('click', '.productSubmitButton', function(e) {
    ...
    if (basePrice <= 0) {
      swal({
        title: "{{ trans('message.Price is required') }}",
        text: "{{ trans('message.Please enter product price') }}",
        icon: 'warning',
        cancelButtonColor: '#C1C1C1',
        buttons: { cancel: msg35 },
        dangerMode: true,
    ...
    if (sellingPrice <= 0) {
      swal({
        title: "{{ trans('message.Selling Price is required') }}",
        text: "{{ trans('message.Please enter selling price') }}",
        icon: 'warning',
        cancelButtonColor: '#C1C1C1',
        buttons: { cancel: msg35 },
        dangerMode: true,
    ...
    if (sellingPrice < basePrice) {
      swal({
        ...
        text: "{{ trans('message.Selling price is less than product price. Do you want to continue?') }}",
        icon: 'warning',
        cancelButtonColor: '#C1C1C1',
        buttons: {
          cancel: "{{ trans('message.No') }}",
          confirm: "{{ trans('message.Yes') }}"
        },
        dangerMode: true,
      }).then((willContinue) => {
        if (!willContinue) {
  ...
  // 🔹 Clear fields when price is cleared
...
<!-- {!! JsValidator::formRequest('App\Http\Requests\ProductAddEditFormRequest', '#productEdit-Form') !!} -->
  {!! preg_replace('/<script\b(?![^>]*\bnonce=)[^>]*>/i', '<script nonce="'.e($cspNonce).'">', (string) JsValidator::formRequest('App\Http\Requests\ProductAddEditFormRequest', '#productAdd-Form'), 1) !!}
