							...
							<div class="content-form-login-page-school-plugin md-form">
								 ...
								 @if(session('message'))
									@php
										...
										$isError = strpos(session('message'), 'not match') !== false || 
												strpos(session('message'), 'Error') !== false;
										$alertClass = $isError ? 'alert-danger' : 'alert-success';
										$iconClass = $isError ? 'fa-exclamation-circle' : 'fa-check-circle';
									@endphp
									<div class="alert {{ $alertClass }} alert-dismissible" role="alert" id="flash-message-{{ uniqid() }}">
										<i class="fa {{ $iconClass }}"></i> <strong>{{ session('message') }}</strong>
