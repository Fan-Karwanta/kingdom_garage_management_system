...
<script nonce="{{ $cspNonce }}">
  ...
  // If product exists in database (has pid), delete from DB
  if (product_id && product_id != '' && product_id != 'undefined') {
    $.ajax({
      type: 'GET',
      url: url,
      data: {
        procuctid: product_id
      },
      success: function(response) {
    ...
    // New row that hasn't been saved yet, just remove from DOM
...
{!! preg_replace('/<script\b(?![^>]*\bnonce=)[^>]*>/i', '<script nonce="'.e($cspNonce).'">', (string) JsValidator::formRequest('App\Http\Requests\PurchaseAddEditFormRequest', '#purchaseAdd-Form'), 1) !!}
