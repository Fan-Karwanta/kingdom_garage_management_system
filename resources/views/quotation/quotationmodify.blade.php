                    ...
                    <div class="col-md-12 col-xs-12 col-sm-12 panel-group">
                        <div class="panel panel-default">
                            ...
                            <div id="collapse1" class="panel-collapse collapse in">
                                <div class="panel-body main_data table-responsive no-padding">
                                    ...
                                    <table class="table table-bordered main_data_points" id="main_data_points" align="center">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="fw-bold" style="">{{ trans('message.Observation Point') }}</th>
                                                <th class="fw-bold" style="">{{ trans('message.Service Charge') }}</th>
                                                <th class="fw-bold" style="">{{ trans('message.Select Product') }}</th>
                                                <th class="fw-bold" style="width:10%">{{ trans('message.Price') }}
                                                    (<?php echo getCurrencySymbols(); ?>)
                                                </th>
                                                <th class="fw-bold" style="width:8%">{{ trans('message.Qty') }}</th>
                                                <th class="fw-bold" style="width:8%">{{ trans('message.Discount') }} (%)</th>
                                                <th class="fw-bold" style="width:8%">{{ trans('message.Tax') }} (%)</th>
                                                <th class="fw-bold" style="width:10%">{{ trans('message.Total') }}
                                                    (<?php echo getCurrencySymbols(); ?>)</th>
                                                <th class="fw-bold" style="">{{ trans('message.Chargeable') }}</th>
                                                <th class="fw-bold" style="">{{ trans('message.Comments') }}</th>
                                                <th class="fw-bold" style="">{{ trans('message.Action') }}</th>
                                            </tr>
                                        </thead>
                                        ...
                                        <tbody id="tbd">
                                            ...
                                            <?php foreach ($data as $datas) {   ?>
                                                <tr class="obs_point_data" id="<?php echo 'row_id_delete_' . $i; ?>">
                                                    ...
                                                    <td>
                                                        <input type="number" min="0" max="100" step="0.01" name="product2[discount][]" value="<?php echo $datas->discount ?? 0; ?>" class="form-control discount discount_<?php echo $i; ?>" row_id="{{ $i }}">
                                                    </td>

                                                    <td>
                                                        <input type="number" min="0" max="100" step="0.01" name="product2[tax][]" value="<?php echo $datas->tax ?? 0; ?>" class="form-control tax tax_<?php echo $i; ?>" row_id="{{ $i }}">
                                                    </td>
...
<script nonce="{{ $cspNonce }}">
    $(document).ready(function () {
                ...
                const newRow = `
                    <tr id="row-${categoryId}" class="obs_point_data">
                        ...
                        <td>
                         ...
                         <td>
                            <input type="number" name="product2[discount][]" class="form-control discount" data-row-index="${rowIndex}" value="0" min="0" max="100" step="0.01" />
                        </td>
                        <td>
                            <input type="number" name="product2[tax][]" class="form-control tax" data-row-index="${rowIndex}" value="0" min="0" max="100" step="0.01" />
                        </td>
       ...
       $(document).on('keyup change', '.qtyt, .charge, .prices, .discount, .tax', function () {
            ...
            const discount = parseFloat($row.find('.discount').val()) || 0;
            const tax = parseFloat($row.find('.tax').val()) || 0;
            console.log(price, qty, serviceCharge);
            updateTotalPrice(rowIndex, price, serviceCharge, qty, discount, tax);
        });
        ...
        function updateTotalPrice(rowIndex, price, serviceCharge, qty = 1, discount = 0, tax = 0) {
            const $row = $(`tr:has(input[data-row-index="${rowIndex}"])`);
            
            // const discount = parseFloat($row.find('.discount').val()) || 0;
            // const tax = parseFloat($row.find('.tax').val()) || 0;
            
            // Calculate subtotal (price * qty + service charge)
            const subtotal = (price * qty) + serviceCharge;
...
<script nonce="{{ $cspNonce }}">
    ...
    $(document).ready(function() {
        ...
        // NEW: For old rows - discount change
        $('body').on('change keyup', '.discount', function() {
            var row_id = $(this).attr('row_id');
            var qty = parseFloat($('.qty_' + row_id).val()) || 1;
            var price = parseFloat($('.price_' + row_id).val()) || 0;
            var serviceCharge = parseFloat($('.charge_' + row_id).val()) || 0;
            var discount = parseFloat($(this).val()) || 0;
            var tax = parseFloat($('.tax_' + row_id).val()) || 0;

            // Calculate with discount and tax
            var subtotal = (price * qty) + serviceCharge;
            var discountAmount = subtotal * (discount / 100);
            var afterDiscount = subtotal - discountAmount;
            var taxAmount = afterDiscount * (tax / 100);
            var total = afterDiscount + taxAmount;

            $('.total1_' + row_id).val(total.toFixed(2));
        });

        // NEW: For old rows - tax change
        $('body').on('change keyup', '.tax', function() {
            var row_id = $(this).attr('row_id');
            var qty = parseFloat($('.qty_' + row_id).val()) || 1;
            var price = parseFloat($('.price_' + row_id).val()) || 0;
            var serviceCharge = parseFloat($('.charge_' + row_id).val()) || 0;
            var discount = parseFloat($('.discount_' + row_id).val()) || 0;
